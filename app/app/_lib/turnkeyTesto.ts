...
import { encodePacked, hexToBytes, keccak256, rpc } from "viem/utils";
...
export async function testTheSigning(): Promise<void> {
  ...
  const installOp = await client.installSessionKeyPlugin({
  ...
  console.log("sent install uo:", installOp);
  ...
  const receipt = await client.getUserOperationReceipt(installOp.hash);
  console.log({ receipt });
